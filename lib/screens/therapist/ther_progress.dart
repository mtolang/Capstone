...
class TherProgress extends StatefulWidget {
  const TherProgress({Key? key}) : super(key: key);

  @override
  State<TherProgress> createState() => _TherProgressState();
}

class _TherProgressState extends State<TherProgress> {
  String? therapistId;
  ...
  Future<void> _getTherapistId() async {
    ...
    // Use therapist-specific key lookup
    String? therapistId = prefs.getString('therapist_id');
    print('🔍 therapist_id key result: $therapistId');

    if (therapistId == null) {
      final possibleKeys = ['user_id', 'therapistId', 'userId', 'id', 'ther_id'];
      for (final key in possibleKeys) {
        therapistId = prefs.getString(key);
        print('🔍 Trying key "$key": $therapistId');
        if (therapistId != null) {
          print('✅ Found therapist ID with key "$key": $therapistId');
          break;
        }
      }
    } else {
      print('✅ Found therapist ID with primary key "therapist_id": $therapistId');
    }

    if (therapistId == null) {
      print('❌ No therapist ID found in any key!');
    }
  ...
  Future<void> _loadClientsAndProgress() async {
    if (therapistId == null) {
      print('❌ Therapist ID is null, cannot load data');
      return;
    }
    ...
    try {
      ...
      print('🚀 Starting data loading with therapist ID: $therapistId');
      final startTime = DateTime.now();

      // Load AcceptedBooking, OTAssessments, and Games collections for therapist
      ...
      print('📊 AcceptedBooking query returned: ${patientsSnapshot.docs.length} documents');
      print('📊 OTAssessments query returned: ${assessmentsSnapshot.docs.length} documents');
      ...
      if (patientsSnapshot.docs.isNotEmpty) {
        print('🔬 First AcceptedBooking doc: ${patientsSnapshot.docs.first.data()}');
      }
      if (assessmentsSnapshot.docs.isNotEmpty) {
        print('🔬 First OTAssessment doc: ${assessmentsSnapshot.docs.first.data()}');
      }
      ...
      print('📊 Processing: ${patientsSnapshot.docs.length} bookings, ${assessmentsSnapshot.docs.length} assessments');
      ...
      print('✅ Final: ${clientsList.length} clients, ${allAssessments.length} assessments');
    } catch (e) {
  ...
  void _processAnalyticsData(List<QueryDocumentSnapshot> assessmentDocs) {
    ...
    for (var doc in assessmentDocs) {
      ...
      final assessmentType = data['assessmentType'] ?? 'Speech Therapy';
      progressTypeStats[assessmentType] = (progressTypeStats[assessmentType] ?? 0) + 1;

      // Also count by skill categories
      if (data['fineMotorSkills'] != null) {
        progressTypeStats['Fine Motor'] = (progressTypeStats['Fine Motor'] ?? 0) + 1;
      }
      if (data['grossMotorSkills'] != null) {
        progressTypeStats['Gross Motor'] = (progressTypeStats['Gross Motor'] ?? 0) + 1;
      }
      if (data['sensoryProcessing'] != null) {
        progressTypeStats['Sensory Processing'] = (progressTypeStats['Sensory Processing'] ?? 0) + 1;
      }
      if (data['cognitiveSkills'] != null) {
        progressTypeStats['Cognitive'] = (progressTypeStats['Cognitive'] ?? 0) + 1;
      }
    }
    ...
    for (int day = 0; day < 7; day++) {
      if (dailyScores[day]!.isNotEmpty) {
        final average = dailyScores[day]!.reduce((a, b) => a + b) / dailyScores[day]!.length;
    ...
    if (weeklyProgressScores.every((score) => score == 0.0) && assessmentDocs.isNotEmpty) {
      double totalScore = 0;
      for (var doc in assessmentDocs) {
        final data = doc.data() as Map<String, dynamic>;
      ...
      weeklyProgressScores = List.generate(7, (index) => avgScore + (index - 3) * 2);
    }
  }
  ...
  double _calculateOverallScore(Map<String, dynamic> assessmentData) {
    ...
    if (assessmentData['fineMotorSkills'] != null) {
      ...
      final avg = scores.reduce((a, b) => a + b) / scores.length * 20; // Scale to 100
      totalScore += avg;
      categoryCount++;
    }
  ...
  void _processTherapyAnalytics(List<QueryDocumentSnapshot> assessmentDocs, List<QueryDocumentSnapshot> bookingDocs) {
    // Reset therapy data
    therapyProgressData = {
      'Motor': [],
      'Speech': [],
    ...
    for (var doc in bookingDocs) {
      final data = doc.data() as Map<String, dynamic>;
      final patientId = data['patientId']?.toString() ?? data['patientName']?.toString() ?? '';
    ...
    for (var doc in assessmentDocs) {
      ...
      String therapyType = _mapTherapyType(patientTherapyTypes[patientId] ?? '');
      ...
      if (therapyType == 'Speech' || patientTherapyTypes[patientId]?.toLowerCase().contains('speech') == true) {
    ...
    for (String therapy in therapyScores.keys) {
      if (therapyScores[therapy]!.isNotEmpty) {
        ...
        therapyAverageScores[therapy] = therapyScores[therapy]!.reduce((a, b) => a + b) / therapyScores[therapy]!.length;
      }
    }
  }
  ...
  String _mapTherapyType(String appointmentType) {
    ...
    if (type.contains('speech') || type.contains('language')) {
      return 'Speech';
    } else if (type.contains('occupational') || type.contains('motor') || type.contains('physical')) {
      return 'Motor';
    } else if (type.contains('cognitive') || type.contains('behavioral')) {
      return 'Cognitive';
    }
    return 'Speech'; // Default fallback for therapist
  }
  ...
  String _getGameTherapyType(String gameName) {
    ...
    if (name.contains('talk') || name.contains('tiles') || name.contains('speech')) {
      return 'Speech';
    } else if (name.contains('pattern') || name.contains('master') || name.contains('cognitive')) {
      return 'Cognitive';
    } else if (name.contains('trace') || name.contains('motor') || name.contains('draw')) {
      return 'Motor';
    }
    return 'Speech'; // Default fallback for therapist
  }
  ...
  String _calculateWeeklyChangeByTherapy(String therapyType) {
    ...
    try {
        ...
        try {
          ...
          if (score > 0) {
            if (reportDate.isAfter(thisWeekStart)) {
              ...
              thisWeekCount++;
            } else if (reportDate.isAfter(lastWeekStart) && reportDate.isBefore(thisWeekStart)) {
      ...
      final change = ((thisWeekAvgScore - lastWeekAvgScore) / lastWeekAvgScore * 100);
    ...
    for (var client in clientsWithProgress) {
      try {
        ...
        if (appointmentDateStr != null && appointmentDateStr != 'N/A') {
          ...
          if (appointmentDate.isAfter(tomorrow) && appointmentDate.isBefore(nextWeek)) {
            upcomingCount++;
          }
        }
      } catch (e) {
                                    ...
                                    children: [
                                      ...
                                      Expanded(
                                        child: _buildProgressCategory(
                                          title: 'Speech',
                                          progress: _calculateProgressPercentage('Speech'),
                                          daysOverview: _calculateWeeklyChange('Speech'),
                                          status: _getProgressStatus('Speech'),
                                          statusColor: _getStatusColor('Speech'),
                                          progressColor: _getStatusColor('Speech'),
                                        ),
                                      ),
                                      const SizedBox(width: 15),
                                      // Motor Progress
                                      Expanded(
                                        child: _buildProgressCategory(
                                          title: 'Motor',
                                          progress: _calculateProgressPercentage('Motor'),
                                          daysOverview: _calculateWeeklyChange('Motor'),
                                          status: _getProgressStatus('Motor'),
                                          statusColor: _getStatusColor('Motor'),
                                          progressColor: _getStatusColor('Motor'),
                                        ),
                                      ),
                                    ],
                                  ),
                                  ...
                                  Row(
                                    children: [
                                      ...
                                      Expanded(
                                        child: _buildProgressCategory(
                                          title: 'Cognitive',
                                          progress: _calculateProgressPercentage('Cognitive'),
                                          daysOverview: _calculateWeeklyChange('Cognitive'),
                                          status: _getProgressStatus('Cognitive'),
                                          statusColor: _getStatusColor('Cognitive'),
                                          progressColor: _getStatusColor('Cognitive'),
                                        ),
                                      ),
                                    ],
                                  ),
                  ...
                  children: [
                    Container(
                      ...
                      child: Text(
                        ...
                        style: TextStyle(
                          color: assessmentCount > 0 ? Colors.white : Colors.grey[600],
                          fontSize: 12,
              ...
              child: FutureBuilder<int>(
                ...
                builder: (context, snapshot) {
                  ...
                  return ElevatedButton(
                    onPressed: hasAssessments ? () => _viewClientProgress(client) : null,
                    style: ElevatedButton.styleFrom(
                      backgroundColor: hasAssessments
                          ? const Color(0xFF006A5B)
                          : Colors.grey[300],
                      foregroundColor: hasAssessments ? Colors.white : Colors.grey[600],
  ...
  Future<int> _getOTAssessmentsCount(String? clientId) async {
    ...
    try {
      print('🔍 Getting OT assessments count for clientId: $clientId, therapistId: $therapistId');
      ...
      print('🔍 OT assessments count query returned: ${snapshot.docs.length} documents');
  ...
  void _viewClientProgress(Map<String, dynamic> client) async {
    ...
    // For now, show a simple dialog since we don't have the detailed progress page
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: Text('${client['childName']} Progress'),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('Patient: ${client['childName']}'),
            Text('Parent: ${client['parentName']}'),
            Text('Age: ${client['age']}'),
            Text('Diagnosis: ${client['diagnosis']}'),
            Text('Total Reports: ${client['totalReports']}'),
          ],
        ),
        actions: [
          TextButton(
  ...
  List<Widget> _buildUpcomingSessionsList() {
    ...
    final upcomingSessions = clientsWithProgress
        .where((client) {
          try {
            ...
            if (appointmentDateStr != null && appointmentDateStr != 'N/A') {
              final appointmentDate = DateTime.parse(appointmentDateStr);
              return appointmentDate.isAfter(now) && appointmentDate.isBefore(nextWeek);
            }
          } catch (e) {
            // Skip invalid dates
          }
    ...
    return upcomingSessions.map((session) {
      final sessionType = _getSessionTypeFromDiagnosis(session['diagnosis'] ?? 'Speech Therapy');
  ...
  String _getSessionTypeFromDiagnosis(String diagnosis) {
    ...
    if (diagnosisLower.contains('speech') || diagnosisLower.contains('language')) {
      return 'Speech Therapy';
    } else if (diagnosisLower.contains('motor') || diagnosisLower.contains('physical')) {
      return 'Motor Therapy';
    } else if (diagnosisLower.contains('cognitive') || diagnosisLower.contains('learning')) {
      return 'Cognitive Therapy';
    } else if (diagnosisLower.contains('social') || diagnosisLower.contains('emotional')) {
